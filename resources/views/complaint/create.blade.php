...
<!--   <script>
   ...
   <script>
      ...
      $('#customer_name').on('change', function() {
          if (isProgrammatic) return; // ⭐ prevent loop

          let number = $(this).find(':selected').data('number') || '';
          $('#customer_number').val(number).keyup();
      });


      function hideSelectShowInput() {
          $("#customer_name").next('.select2-container').hide(); // hide select2 box
          $("#customer_name").hide();                            // hide actual select
          $("#custom_customer_name").show(); 
          $("#end_user_id").val("");
         $("#customer_email").val("");
         ...
         $("#customer_name").prop('readonly', false);
         $("#customer_email").prop('readonly', false);                   // show input
      }

      function showSelectHideInput() {
          $("#customer_name").next('.select2-container').show(); // show select2 box
          $("#customer_name").show();                            // show actual select
          $("#custom_customer_name").hide();                     // hide input
      }


      $("#customer_number").on("keyup", function() {
         var customer_number = $(this).val();
         if(customer_number.length < 5){
            showSelectHideInput();
            return ;
         }
         $.ajax({
